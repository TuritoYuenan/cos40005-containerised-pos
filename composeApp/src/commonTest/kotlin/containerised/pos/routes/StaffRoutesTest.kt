...
class StaffRoutesTest {
	...
	fun `route serial names match expected contract`() {
		assertEquals("loading", StaffRoutes.Loading.serializer().descriptor.serialName)
		...
		assertEquals("edit-ingredient", StaffRoutes.EditIngredient.serializer().descriptor.serialName)
		...
		assertEquals("sales", StaffRoutes.Sales.serializer().descriptor.serialName)
		assertEquals("sales-report", StaffRoutes.SalesReport.serializer().descriptor.serialName)
		assertEquals("employee-management", StaffRoutes.EmployeeManagement.serializer().descriptor.serialName)
		assertEquals("employee-profile", StaffRoutes.EmployeeProfile.serializer().descriptor.serialName)
	...
	fun `route serial names are unique`() {
		val serialNames = listOf(
			StaffRoutes.Loading.serializer().descriptor.serialName,
			...
			StaffRoutes.Sales.serializer().descriptor.serialName,
			StaffRoutes.SalesReport.serializer().descriptor.serialName,
			StaffRoutes.EmployeeManagement.serializer().descriptor.serialName,
			StaffRoutes.EmployeeProfile.serializer().descriptor.serialName,
	...
	fun `non-nullable args routes throw when decoding without required fields`() {
		...
		assertFailsWith<SerializationException> {
			json.decodeFromString<StaffRoutes.SalesReport>("{}")
		}
	}
	...
	fun `nullable args routes decode with null when fields are missing`() {
		...
		val employeeProfile = json.decodeFromString<StaffRoutes.EmployeeProfile>("{}")

		assertNull(editItem.itemId)
		assertNull(editPromotion.promotionId)
		assertNull(editTag.tagId)
		assertNull(employeeProfile.argUserId)
	}
