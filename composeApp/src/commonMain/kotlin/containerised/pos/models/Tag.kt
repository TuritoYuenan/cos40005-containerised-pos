...
/**
 * Represents a tag used for categorising menu items.
...
data class Tag(
	...
	companion object {
		suspend fun fetchAll(): List<Tag> = SupabaseClient.db["tags"]
			.select().decodeList<Tag>()

		suspend fun fetchById(id: String): Tag? = SupabaseClient.db["tags"]
			.select {
				filter { eq("tag_id", id) }
				limit(1)
			}
			.decodeList<Tag>().firstOrNull()

		suspend fun deleteById(id: String) = SupabaseClient.db["tags"]
			.delete { filter { eq("tag_id", id) } }
